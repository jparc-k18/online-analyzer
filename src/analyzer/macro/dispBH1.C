...
void dispBH1()
{
  ...
  c->Clear();
  c->Divide(4,3);
  int base_id = HistMaker::getUniqueID(kBH1, 0, kADC, 1+n_seg);
  for(int i = 0; i<n_seg; ++i){
    c->cd(i+1);
    gPad->SetLogy();
    TH1 *h = (TH1*)GHist::get(base_id + i);
  ...
  c->Clear();
  c->Divide(4,3);
  int base_id = HistMaker::getUniqueID(kBH1, 0, kTDC, 1);
  for(int i = 0; i<n_seg; ++i){
    c->cd(i+1);
  ...
  c->Clear();
  c->Divide(4,3);
  int base_id = HistMaker::getUniqueID(kBH1, 0, kTDC, 1+n_seg);
  for(int i = 0; i<n_seg; ++i){
    c->cd(i+1);
    GHist::get(base_id + i)->Draw();
  }
