...
# clhep_cpp     := $(shell $(clhep_config) --cpp)
# clhep_cxx     := $(shell $(clhep_config) --cxx)
clhep_include := $(shell $(clhep_config) --include)
clhep_libs    := $(shell $(clhep_config) --libs)
clhep_ldflags := $(shell $(clhep_config) --ldflags)
...
#### HD DAQ Unpacker ####
unpacker_install      := $(HOME)/software/unpacker/pro
#unpacker_install      := $(HOME)/software/unpacker/unpacker.tmp
...
define obj-to-lib
 ...
 $(strip $1)
	$(QUIET) $(ECHO) "===== create granular library";
	$(QUIET) $(ECHO) "      $$^";
	$(QUIET) $(ECHO) "      ->    $$@";
...
$1:: \
 ...
 $(subst dict,linkdef,$(subst _Dict.cc,_LinkDef.hh,$1))
	$(QUIET) $(ECHO) "==== Generating dictionary $$@ from $$^";
	rootcint -f $$@ -c -p $$^;
endef
