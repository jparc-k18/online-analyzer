...
PrintScalerSheet( void )
{
  ...
  DrawOneLine( "BAC1", "K_out", "PS-BH1OR" );
  DrawOneLine( "BAC2", "pi_out", "TOF-24" );
...
process_begin(const std::vector<std::string>& argv)
{
  ...
  { // kLeft column (counter info)
    ...
    info[kLeft][index++] = ScalerInfo("TOF-24",  0, 31, true);
    info[kLeft][index++] = ScalerInfo("PS-BH1OR",0, 30, true);
