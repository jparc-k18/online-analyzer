...
process_event( void )
{
  ...
  //------------------------------------------------------------------
  {
    ...
    if( nhbh1_flag>0 && nhbh2_flag>0 && nhbh1_flag<3 && nhbh2_flag<3 && -0.2<btof && btof<0.8 ){
      pi_flag=1;
    ...
    //if( nhbh1_flag>0 && nhbh2_flag>0 && nhbh1_flag<3 && nhbh2_flag<3 && -2.8<btof && btof<-1.8 ){  // 1.4GeV/c
      ...
      hptr_array[btof_id+5+19+8]->Fill(0); //PVACeff
  ...
  // // BFT -----------------------------------------------------------
  {
    ...
    for(int i = 0; i<NumOfSegBFT; ++i){
	  ...
	  if(k_flag==1){
	    static const int btof_id  = gHist.getSequentialID(kMisc, 0, kTDC);
	    hptr_array[btof_id+30+16+2]->Fill(i);
	  }
  	}
	  ...
	  if(k_flag==1){
	    static const int btof_id  = gHist.getSequentialID(kMisc, 0, kTDC);
	    hptr_array[btof_id+30+16+3]->Fill(i);
	  }
  	}
  	if(tdc_prev==tdc) continue;
  	tdc_prev = tdc;
