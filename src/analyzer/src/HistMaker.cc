...
TList *HistMaker::createBC3(Bool_t flag_ps)
{
  ...
  // TDC---------------------------------------------------------
  {
    ...
    // TDCwide
    target_id = getUniqueID(kBC3, 0, kTDC, 10);
    for (Int_t i = 0; i < NumOfLayersBC3; ++i)
    {
      const char *title = NULL;
      title = Form("%s_%swide_%s", nameDetector, nameSubDir, name_layer[i]);
      sub_dir->Add(createTH1(target_id + i + 1, title, // 1 origin
                             5000, 0, 5000,
                             "TDC [ch]", ""));
    }
...
TList *HistMaker::createBC4(Bool_t flag_ps)
{
  ...
  const char *name_Selflayer[NumOfDimBC4] = {"vu0_u1", "v0_v1", "x0_x1"};
  //  const char* name_layer[NumOfLayersSDC3] = { "y0", "y1", "x0", "x1" };
  ...
  // TDC---------------------------------------------------------
  {
    ...
    // TDCwide
    target_id = getUniqueID(kBC4, 0, kTDC, 10);
    for (Int_t i = 0; i < NumOfLayersBC4; ++i)
    {
      const char *title = NULL;
      title = Form("%s_%swide_%s", nameDetector, nameSubDir, name_layer[i]);
...
TList *HistMaker::createBH2(Bool_t flag_ps)
{
  ...
  // TDC ----------------------------------------------------
  {
    ...
    // BH2MTLR(BH2 mean timer low reso.) ----------------------------------------------------
    // Make histogram and add it
...
TList *HistMaker::createHTOF(Bool_t flag_ps)
{
  ...
  // Hit parttern -----------------------------------------------
  {
    ...
    auto h_hit_poly = dynamic_cast<TH2Poly *>(createTH2Poly(++target_id, title, -400, 400, -400, 400));
...
TList *HistMaker::createE75BAC(Bool_t flag_ps)
{
  ...
  const char *name_acs[] = {"E75FMBAC1", "E75FMBAC2"};
...
TList *HistMaker::createE75LFBAC(Bool_t flag_ps)
{
  ...
  // ADC ---------------------------------------------------------
  {
    ...
    Int_t target_id = getUniqueID(kE75LFBAC, 0, kADC, 0);
    for (Int_t i = 0; i < NumOfSegE75LFBAC; ++i)
    {
      const char *title = NULL;
      title = Form("%s_%s", name_acs[i], nameSubDir);
      sub_dir->Add(createTH1(++target_id, title, // 1 origin
  ...
  // ADC w/TDC ---------------------------------------------------------
  {
    ...
    Int_t target_id = getUniqueID(kE75LFBAC, 0, kADCwTDC, 0);
    for (Int_t i = 0; i < NumOfSegE75LFBAC; ++i)
    {
      const char *title = NULL;
      title = Form("%s_%s", name_acs[i], nameSubDir);
  ...
  // TDC---------------------------------------------------------
  {
    ...
    Int_t target_id = getUniqueID(kE75LFBAC, 0, kTDC, 0);
    for (Int_t i = 0; i < NumOfSegE75LFBAC; ++i)
    {
      const char *title = NULL;
      title = Form("%s_%s", name_acs[i], nameSubDir);
  ...
  { // Hit parttern -----------------------------------------------
    Int_t target_id = getUniqueID(kE75LFBAC, 0, kHitPat, 0);
    top_dir->Add(createTH1(++target_id, "E75LFBAC_hit_pattern",
                           NumOfSegE75LFBAC, 0, NumOfSegE75LFBAC,
                           "Segment", ""));
  }

  { // Multiplicity -----------------------------------------------
    Int_t target_id = getUniqueID(kE75LFBAC, 0, kMulti, 0);
    for (Int_t i = 0; i < NumOfSegE75LFBAC; ++i)
    {
      const char *title = NULL;
      title = Form("%s_%s", name_acs[i], "multiplicity");
      top_dir->Add(createTH1(++target_id, title,
                             NumOfSegE75LFBAC + 1, 0, NumOfSegE75LFBAC + 1,
                             "Multiplicity", ""));
    }
  }
  ...
  // TDC---------------------------------------------------------
  {
    ...
    for (Int_t ud = 0; ud < 2; ud++)
    {
      for (Int_t i = 0; i < NumOfPlaneAFT; ++i)
      {
        ...
        if (ud == 0)
          title = Form("%s_%s_%dU_%s", nameDetector, sub_name, i / 4 + 1, layer_name.Data());
        if (ud == 1)
          title = Form("%s_%s_%dD_%s", nameDetector, sub_name, i / 4 + 1, layer_name.Data());
  ...
  // TDC-2D -------------------------------------------------------
  {
    ...
    for (Int_t ud = 0; ud < 2; ud++)
    {
      for (Int_t i = 0; i < NumOfPlaneAFT; ++i)
      {
        ...
        const TString layer_name = NameOfPlaneAFT[i % 4];
        if (ud == 0)
          title = Form("%s_%s_%dU_%s_2D", nameDetector, sub_name, i / 4 + 1, layer_name.Data());
        if (ud == 1)
          title = Form("%s_%s_%dD_%s_2D", nameDetector, sub_name, i / 4 + 1, layer_name.Data());
  ...
  // Hit parttern -----------------------------------------------
  {
    ...
    for (Int_t ud = 0; ud < 3; ud++)
    {
      for (Int_t i = 0; i < NumOfPlaneAFT; ++i)
      {
        ...
        if (ud == 1)
          ...
          title = Form("%s_%s_%d_%s", nameDetector, sub_name, i / 4 + 1, layer_name.Data()); // for hitpattern defined by hits on both edges
    ...
    for (int iPlane = 0; iPlane < NumOfPlaneAFT; iPlane++)
    {
      for (int iSeg = 0; iSeg < NumOfSegAFT[iPlane % 4]; iSeg++)
      {
        ...
        if (iPlane % 4 == 0 || iPlane % 4 == 1)
          h_hit_poly_x->AddBin(npoly, Z, X);
        if (iPlane % 4 == 2 || iPlane % 4 == 3)
          h_hit_poly_y->AddBin(npoly, Z, X);
      }
    }
...
TList *HistMaker::createSSD0(Bool_t flag_ps)
{
  ...
  // Hit parttern -----------------------------------------------
  {
    ...
    sub_dir->Add(createTH2(++target_id, Form("%s_%s_%s%%%s", nameDetector, nameSubDir, nameLayer[1], nameLayer[0]),
...
TList *HistMaker::createSSD1(Bool_t flag_ps)
{
  ...
  // Hit parttern -----------------------------------------------
  {
    ...
    sub_dir->Add(createTH2(++target_id, Form("%s_%s_%s%%%s", nameDetector, nameSubDir, nameLayer[0], nameLayer[1]),
                           NumOfSegSSD1 / 12, 0, NumOfSegSSD1, NumOfSegSSD1 / 12, 0, NumOfSegSSD1,
                           nameLayer[1], nameLayer[0]));
    sub_dir->Add(createTH2(++target_id, Form("%s_%s_%s%%%s", nameDetector, nameSubDir, nameLayer[2], nameLayer[3]),
                           NumOfSegSSD1 / 12, 0, NumOfSegSSD1, NumOfSegSSD1 / 12, 0, NumOfSegSSD1,
...
TList *HistMaker::createSSD2(Bool_t flag_ps)
{
  ...
  // Hit parttern -----------------------------------------------
  {
    ...
    sub_dir->Add(createTH2(++target_id, Form("%s_%s_%s%%%s", nameDetector, nameSubDir, nameLayer[1], nameLayer[0]),
                           ...
                           nameLayer[0], nameLayer[1]));
    sub_dir->Add(createTH2(++target_id, Form("%s_%s_%s%%%s", nameDetector, nameSubDir, nameLayer[3], nameLayer[2]),
                           NumOfSegSSD2 / 12, 0, NumOfSegSSD2, NumOfSegSSD2 / 12, 0, NumOfSegSSD2,
...
TList *HistMaker::createSDC1(Bool_t flag_ps)
{
  ...
  const char *name_Selflayer[NumOfDimSDC1] = {"u0_u1", "x0_x1", "v0_v1"};
  //  const char* name_layer[NumOfLayersSDC3] = { "y0", "y1", "x0", "x1" };
  ...
  // TDC---------------------------------------------------------
  {
    ...
    // TDCwide
    target_id = getUniqueID(kSDC1, 0, kTDC, 10);
    for (Int_t i = 0; i < NumOfLayersSDC1; ++i)
    {
      const char *title = NULL;
      title = Form("%s_%swide_%s", nameDetector, nameSubDir, name_layer[i]);
...
TList *HistMaker::createSDC3(Bool_t flag_ps)
{
  ...
  // TDC---------------------------------------------------------
  {
    ...
    // TDC 2D ---------------------------------------------------
    target_id = getUniqueID(kSDC3, 0, kTDC2D, 20);
    for (Int_t i = 0; i < NumOfLayersSDC3; ++i)
    {
      const char *title = NULL;
      title = Form("%s_%s2D_%s", nameDetector, nameSubDir, name_layer[i]);
...
TList *HistMaker::createSDC4(Bool_t flag_ps)
{
  ...
  // TDC---------------------------------------------------------
  {
    ...
    // TDC 2D ---------------------------------------------------
    target_id = getUniqueID(kSDC4, 0, kTDC2D, 20);
    for (Int_t i = 0; i < NumOfLayersSDC4; ++i)
    {
      const char *title = NULL;
      title = Form("%s_%s2D_%s", nameDetector, nameSubDir, name_layer[i]);
...
TList *HistMaker::createSDC5(Bool_t flag_ps)
{
  ...
  // TDC---------------------------------------------------------
  {
    ...
    // TDC 2D ---------------------------------------------------
    target_id = getUniqueID(kSDC5, 0, kTDC2D, 20);
    for (Int_t i = 0; i < NumOfLayersSDC5; ++i)
    {
      const char *title = NULL;
      title = Form("%s_%s2D_%s", nameDetector, nameSubDir, name_layer[i]);
      Int_t NumOfWireSDC5 = 0;
      if (name_layer[i] == "y0" || name_layer[i] == "y1")
        NumOfWireSDC5 = NumOfWireSDC5Y;
      if (name_layer[i] == "x0" || name_layer[i] == "x1")
  ...
  // HitPat------------------------------------------------------
  {
    ...
    {
      if (i == 0 || i == 1)
        nwire = NumOfWireSDC5Y;
      if (i == 2 || i == 3)
        nwire = NumOfWireSDC5X;
      const char *title = NULL;
      title = Form("%s_%s_%s", nameDetector, nameSubDir, name_layer[i]);
      sub_dir->Add(createTH1(target_id + i + 1, title, // 1 origin
                             nwire, 0, nwire,
...
HistMaker::createTPC(Bool_t flag_ps)
{
  ...
  top_dir->SetName(nameDetector);
  {
    ...
    auto h_adc = dynamic_cast<TH2Poly *>(createTH2Poly(target_id++, title, -300, 300, -300, 300));
    title = Form("%s_RMS2D", nameDetector);
    auto h_rms = dynamic_cast<TH2Poly *>(createTH2Poly(target_id++, title, -300, 300, -300, 300));
    title = Form("%s_TDC2D", nameDetector);
    auto h_loc = dynamic_cast<TH2Poly *>(createTH2Poly(target_id++, title, -300, 300, -300, 300));
    title = Form("%s_Hit2D", nameDetector);
    auto h_hit = dynamic_cast<TH2Poly *>(createTH2Poly(target_id++, title, -300, 300, -300, 300));
    Double_t X[5];
    Double_t Y[5];
...
HistMaker::createBeamProfileE42(Bool_t flag_ps)
{
  ...
  for (Int_t ip = 0; ip < NParticleType; ++ip)
  {
    ...
    sub_dir->Add(createTH2(unique_id++, Form("BcOut FF 0 U%%X (%s)", ParticleName[ip].Data()),
                           100, -200., 200., 100, -0.2, 0.2,
                           "x position [mm]", "u0"));
    sub_dir->Add(createTH2(unique_id++, Form("BcOut FF 0 V%%Y (%s)", ParticleName[ip].Data()),
...
TList *HistMaker::createDAQ(Bool_t flag_ps)
{
  ...
  { //___ ParaDAQ
    ...
    {
      // Declaration of the sub-directory
      TString strSubSubDir = CONV_STRING(kTDC);
    ...
    // QDCvsTOT ---------------------------------------------------------
    {
      // Declaration of the sub-directory
      {
        ...
        // Hit parttern -----------------------------------------------
        {
          const char *title = "TOF_hit_pattern_2";
          Int_t target_id = getUniqueID(kTMCC, 0, kHitPat, 0);
          // Add to the top directory
...
TList *HistMaker::createPWO_E05(Bool_t flag_ps)
{
  ...
  // ADC---------------------------------------------------------
  {
    ...
    {
      sub_dir->Add(createTH1(++target_id, Form("%s_%s_%d", nameDetector, nameSubDir, i + 1),
                             0x1000, 0, 0x1000,
                             "ADC [ch]", ""));
    }
    // insert sub directory
...
// createRC
// -------------------------------------------------------------------------
TList *HistMaker::createRC(Bool_t flag_ps)
{

  // Determine the detector name
  TString strDet = CONV_STRING(kRC);
  // name list of crearted detector
  name_created_detectors_.push_back(strDet);
  if (flag_ps)
  {
  ...
  //   Int_t target_id = getUniqueID(kRC, 0, kTDC, 0);
  //   const char *sub_name = "TDC";
  //   // Add to the top directory
  //   for (Int_t i = 0; i < NumOfPlaneRC; ++i)
  //   {
  //     for (Int_t seg = 0; seg < NumOfSegRC[i]; seg++)
  //     {
  ...
  //         if (ud == 0)
  //           title = Form("%s%s_%dU_%s", nameDetector, layer_name.Data(), seg, sub_name);
  //         if (ud == 1)
  //           title = Form("%s%s_%dD_%s", nameDetector, layer_name.Data(), seg, sub_name);
  ...
  //   { // TOT---------------------------------------------------------
  //     Int_t target_id = getUniqueID(kRC, 0, kTOT, 0);
  //     const char *sub_name = "TOT";
  //     // Add to the top directory
  //     for (Int_t i = 0; i < NumOfPlaneRC; ++i)
  //     {
  ...
  //   { // CTOT---------------------------------------------------------
  //     Int_t target_id = getUniqueID(kRC, 0, kTOT, 200);
  //     const char *sub_name = "CTOT";
  //     // Add to the top directory
  //     for (Int_t i = 0; i < NumOfPlaneRC; ++i)
  //     {
  ...
  //   Int_t target_id = getUniqueID(kRC, 0, kHighGain, 0);
  //   const char *sub_name = "HG";
  //   // Add to the top directory
  //   for (Int_t i = 0; i < NumOfPlaneRC; ++i)
  //   {
  ...
  //   // ADC HighGain Cut -------------------------------------------
  //   target_id = getUniqueID(kRC, 0, kHighGain, 200);
  //   sub_name = "CHG";
  //   // Add to the top directory
  //   for (Int_t i = 0; i < NumOfPlaneRC; ++i)
  //   {
  ...
  //   Int_t target_id = getUniqueID(kRC, 0, kLowGain, 0);
  //   const char *sub_name = "LG";
  //   // Add to the top directory
  //   for (Int_t i = 0; i < NumOfPlaneRC; ++i)
  //   {
  ...
  //   // ADC LowGain Cut --------------------------------------------
  //   target_id = getUniqueID(kRC, 0, kLowGain, 200);
  //   sub_name = "CLG";
  //   // Add to the top directory
  //   for (Int_t i = 0; i < NumOfPlaneRC; ++i)
  //   {
  ...
  // TDC-2D -------------------------------------------------------
  {
    ...
    Int_t target_id = getUniqueID(kRC, 0, kTDC2D, 0);
    const char *sub_name = "TDC";
    // Add to the top directory
    for (Int_t i = 0; i < NumOfPlaneRC; ++i)
    {
      for (Int_t ud = 0; ud < NumOfUorDRC[i]; ud++)
      {
        ...
        const TString layer_name = NameOfPlaneRC[i];
        if (ud == 0)
          title = Form("%s%s_%sU_2D", nameDetector, layer_name.Data(), sub_name);
        if (ud == 1)
          title = Form("%s%s_%sD_2D", nameDetector, layer_name.Data(), sub_name);
  ...
  // TOT-2D --------------------------------------------------------
  {
    ...
    Int_t target_id = getUniqueID(kRC, 0, kTOT2D, 0);
    const char *sub_name = "TOT";
    // Add to the top directory
    for (Int_t i = 0; i < NumOfPlaneRC; ++i)
    {
    ...
    // CTOT-2D (after cut) --------------------------------------------
    target_id = getUniqueID(kRC, 0, kTOT2D, 100);
    sub_name = "CTOT";
    // Add to the top directory
    for (Int_t i = 0; i < NumOfPlaneRC; ++i)
    {
  ...
  // ADC-2D HighGain ----------------------------------------------
  {
    ...
    Int_t target_id = getUniqueID(kRC, 0, kHighGain2D, 0);
    const char *sub_name = "HG";
    // Add to the top directory
    for (Int_t i = 0; i < NumOfPlaneRC; ++i)
    {
    ...
    // ADC-2D HighGain Cut ------------------------------------------
    target_id = getUniqueID(kRC, 0, kHighGain2D, 100);
    sub_name = "CHG";
    // Add to the top directory
    for (Int_t i = 0; i < NumOfPlaneRC; ++i)
    {
  ...
  // ADC-2D LowGain -----------------------------------------------
  {
    ...
    Int_t target_id = getUniqueID(kRC, 0, kLowGain2D, 0);
    const char *sub_name = "LG";
    // Add to the top directory
    for (Int_t i = 0; i < NumOfPlaneRC; ++i)
    {
    ...
    // ADC-2D LowGain Cut -------------------------------------------
    target_id = getUniqueID(kRC, 0, kLowGain2D, 100);
    sub_name = "CLG";
    // Add to the top directory
    for (Int_t i = 0; i < NumOfPlaneRC; ++i)
    {
  ...
  //   Int_t target_id = getUniqueID(kRC, 0, kHighGainXTDC, 0);
  //   const char *sub_name = "HGXTDC";
  //   // Add to the top directory
  //   for (Int_t i = 0; i < NumOfPlaneRC; ++i)
  //   {
  ...
  //         sub_dir->Add(createTH2(target_id + i + seg + ud + 1, title, // 1 origin
  //                                4096 / 8, 0, 4096,
  //                                // 1024, 0, 1024,
  //                                4096, 0, 4096,
  //                                "TDC [ch]", "HighGain [ch]"));
  ...
  // Hit parttern -----------------------------------------------
  {
    ...
    Int_t target_id = getUniqueID(kRC, 0, kHitPat, 0);
    const char *sub_name = "HitPat";
    // Add to the top directory
    for (Int_t i = 0; i < NumOfPlaneRC; ++i)
    {
    ...
    // Hit parttern (after cut) -----------------------------------
    target_id = getUniqueID(kRC, 0, kHitPat, 100);
    sub_name = "CHitPat";
    // Add to the top directory
    for (Int_t i = 0; i < NumOfPlaneRC; ++i)
    {
    ...
    // with adc
    target_id = getUniqueID(kRC, 0, kHitPat, 200);
    sub_name = "CHitPat_wa";
    // Add to the top directory
    for (Int_t i = 0; i < NumOfPlaneRC; ++i)
    {
  ...
  // Multiplicity -----------------------------------------------
  {
    ...
    Int_t target_id = getUniqueID(kRC, 0, kMulti, 0);
    const char *sub_name = "Multi";
    // Add to the top directory
    for (Int_t i = 0; i < 2; ++i) // -X, +X
    {
      TString layer_name = "RC";
      if (i == 0)
        layer_name.Append("-X");
      if (i == 1)
        layer_name.Append("+X");
      const char *title = Form("%s_%s_%s", nameDetector, sub_name, layer_name.Data());
      Int_t rc_nseg_all = 24;
      sub_dir->Add(createTH1(++target_id, title, // 1 origin
                             rc_nseg_all + 1, 0, rc_nseg_all + 1,
                             "Plane", ""));
    }
    // insert sub directory
    top_dir->Add(sub_dir);
...
// createTF_TF
// -------------------------------------------------------------------------
TList *HistMaker::createTF_TF(Bool_t flag_ps)
{
  // Determine the detector name
  TString strDet = CONV_STRING(kTF_TF);
  // name list of crearted detector
  name_created_detectors_.push_back(strDet);
  if (flag_ps)
  {
  ...
  // ADC---------------------------------------------------------
  {
    ...
    // Make histogram and add it
    Int_t target_id = getUniqueID(kTF_TF, 0, kADC, 0);
    for (Int_t i = 0; i < NumOfSegTF_TF; ++i)
    {
      const char *title = NULL;
      title = Form("%s_%s", nameDetector, nameSubDir);
      sub_dir->Add(createTH1(target_id + i + 1, title, // 1 origin
  ...
  // TDC ---------------------------------------------------------
  {
    ...
    // Make histogram and add it
    Int_t target_id = getUniqueID(kTF_TF, 0, kTDC, 0);
    for (Int_t i = 0; i < NumOfSegTF_TF; ++i)
    {
      const char *title = NULL;
      title = Form("%s_%s", nameDetector, nameSubDir);
...
// createTF_GN1
// -------------------------------------------------------------------------
TList *HistMaker::createTF_GN1(Bool_t flag_ps)
{
  // Determine the detector name
  TString strDet = CONV_STRING(kTF_GN1);
  // name list of crearted detector
  name_created_detectors_.push_back(strDet);
  if (flag_ps)
  {
  ...
  // TDC ---------------------------------------------------------
  {
    ...
    // Make histogram and add it
    Int_t target_id = getUniqueID(kTF_GN1, 0, kTDC, 0);
    for (Int_t i = 0; i < NumOfSegTF_GN1; ++i)
    {
      const char *title = NULL;
      title = Form("%s_%s", nameDetector, nameSubDir);
      sub_dir->Add(createTH1(target_id + i + 1, title, // 1 origin
...
// createTF_GN2
// -------------------------------------------------------------------------
TList *HistMaker::createTF_GN2(Bool_t flag_ps)
{
  // Determine the detector name
  TString strDet = CONV_STRING(kTF_GN2);
  // name list of crearted detector
  name_created_detectors_.push_back(strDet);
  if (flag_ps)
  {
  ...
  // TDC ---------------------------------------------------------
  {
    ...
    // Make histogram and add it
    Int_t target_id = getUniqueID(kTF_GN2, 0, kTDC, 0);
    for (Int_t i = 0; i < NumOfSegTF_GN2; ++i)
    {
      const char *title = NULL;
      title = Form("%s_%s", nameDetector, nameSubDir);
      sub_dir->Add(createTH1(target_id + i + 1, title, // 1 origin
...
// -------------------------------------------------------------------------
TList *
HistMaker::createParaBGO(Bool_t flag_ps)
{
  TString strDet = "ParaBGO";
  name_created_detectors_.push_back(strDet);
  if (flag_ps)
  {
    name_ps_files_.push_back(strDet);
  ...
  { ///// ADCwTDC
    Int_t target_id = getUniqueID(kParaBGO, 0, kADCwTDC);
    for (Int_t i = 0; i < NumOfSegParaBGO; ++i)
    {
  ...
  { ///// TDC
    Int_t target_id = getUniqueID(kParaBGO, 0, kTDC);
    for (Int_t i = 0; i < NumOfSegParaBGO; ++i)
    {
...
// -------------------------------------------------------------------------
TList *
HistMaker::createParaTMC(Bool_t flag_ps)
{
  TString strDet = "ParaTMC";
  name_created_detectors_.push_back(strDet);
  if (flag_ps)
  {
    name_ps_files_.push_back(strDet);
  ...
  { ///// TDC
    ...
    auto title = Form("%s_TDC", nameDetector);
    top_dir->Add(createTH1(target_id, title,
                           100000, 0, 400000,
                           "TDC [ch]", ""));
  }
...
// createParaTC
// -------------------------------------------------------------------------
TList *HistMaker::createParaTC(Bool_t flag_ps)
{
  // Determine the detector name
  TString strDet = CONV_STRING(kParaTC);
  // name list of crearted detector
  name_created_detectors_.push_back(strDet);
  if (flag_ps)
  {
  ...
  // TDC-2D -------------------------------------------------------
  {
    ...
    Int_t target_id = getUniqueID(kParaTC, 0, kTDC2D, 0);
    const char *sub_name = "TDC";
    // Add to the top directory
  ...
  // ADC-2D HighGain ----------------------------------------------
  {
    ...
    Int_t target_id = getUniqueID(kParaTC, 0, kHighGain, 10);
    const char *sub_name = "HG";
    // Add to the top directory
    for (Int_t i = 0; i < NumOfPlaneParaTC; ++i)
    {
    ...
    // ADC-2D HighGain Cut ------------------------------------------
    target_id = getUniqueID(kParaTC, 0, kHighGain, 100);
    sub_name = "CHG";
    // Add to the top directory
    for (Int_t i = 0; i < NumOfPlaneParaTC; ++i)
    {
  ...
  // ADC-2D LowGain -----------------------------------------------
  {
    ...
    Int_t target_id = getUniqueID(kParaTC, 0, kLowGain, 10);
    const char *sub_name = "LG";
    // Add to the top directory
    for (Int_t i = 0; i < NumOfPlaneParaTC; ++i)
    {
    ...
    // ADC-2D LowGain Cut ------------------------------------------
    target_id = getUniqueID(kParaTC, 0, kLowGain, 100);
    sub_name = "CLG";
    // Add to the top directory
    for (Int_t i = 0; i < NumOfPlaneParaTC; ++i)
    {
  ...
  // MultiHitTdc-2D -------------------------------------------------------
  {
    ...
    Int_t target_id = getUniqueID(kParaTC, 0, kMultiHitTdc, 20);
    const char *sub_name = "MultiHitTdc";
    // Add to the top directory
    for (Int_t i = 0; i < NumOfPlaneParaTC; ++i)
    {
  ...
  // ADC HighGain vs TOT ----------------------------------------------
  {
    ...
    Int_t target_id = getUniqueID(kParaTC, 0, kHighGainvsTOT, 30);
    const char *sub_name = "HGvsTOT";
    // Add to the top directory
    for (Int_t i = 0; i < NumOfPlaneParaTC; ++i)
    {
      ...
      {
        const char *title = NULL;
        title = Form("%s%d_%s_%d", nameDetector, i + 1, sub_name, j);
        sub_dir->Add(createTH2(++target_id, title, // 1 origin
                               4096, 0, 4096,
                               150, 0, 150,
                               "ADC [ch]", "TOT [ch]"));
  ...
  // TDC vs TOT ----------------------------------------------
  {
    ...
    Int_t target_id = getUniqueID(kParaTC, 0, kTOTXTDC, 30);
    const char *sub_name = "TDCvsTOT";
    // Add to the top directory
    for (Int_t i = 0; i < NumOfPlaneParaTC; ++i)
    {
      for (Int_t j = 0; j < NumOfSegVMEEASIROC; ++j)
  ...
  // Hit parttern -----------------------------------------------
  {
    ...
    {
      char *direction;
      for (Int_t j = 0; j < NumOfPlaneParaTC; ++j)
      { // X,Y
        const char *title = NULL;
        if (j == 0)
          direction = "X";
  ...
  // // Multiplicity -------------------------------------------------
  {
    ...
    Int_t target_id = getUniqueID(kParaTC, 0, kMulti, 0);
    const char *sub_name = "Multiplicity";
    // Add to the top directory
    for (Int_t i = 0; i < NumOfPlaneParaTC; ++i)
    {
...
// createParaVC
// -------------------------------------------------------------------------
TList *HistMaker::createParaVC(Bool_t flag_ps)
{
  // Determine the detector name
  TString strDet = CONV_STRING(kParaVC);
  // name list of crearted detector
  name_created_detectors_.push_back(strDet);
  if (flag_ps)
  {
  ...
  // TDC-2D -------------------------------------------------------
  {
    ...
    Int_t target_id = getUniqueID(kParaVC, 0, kTDC2D, 0);
    const char *sub_name = "TDC";
    // Add to the top directory
    const char *title = NULL;
    title = Form("%s_%s", nameDetector, sub_name);
  ...
  // TOT-2D -------------------------------------------------------
  {
    ...
    Int_t target_id = getUniqueID(kParaVC, 0, kTOT2D, 0);
    const char *sub_name = "TOT";
    // Add to the top directory
    const char *title = NULL;
    title = Form("%s_%s", nameDetector, sub_name);
  ...
  // ADC-2D HighGain ----------------------------------------------
  {
    ...
    Int_t target_id = getUniqueID(kParaVC, 0, kHighGain, 10);
    const char *sub_name = "HG";
    // Add to the top directory
    const char *title = NULL;
    title = Form("%s_%s", nameDetector, sub_name);
    ...
    // ADC-2D HighGain Cut ------------------------------------------
    target_id = getUniqueID(kParaVC, 0, kHighGain, 100);
    sub_name = "CHG";
    // Add to the top directory
    title = Form("%s_%s", nameDetector, sub_name);
    sub_dir->Add(createTH2(++target_id, title, // 1 origin
  ...
  // ADC-2D LowGain -----------------------------------------------
  {
    ...
    Int_t target_id = getUniqueID(kParaVC, 0, kLowGain, 10);
    const char *sub_name = "LG";
    // Add to the top directory
    const char *title = NULL;
    title = Form("%s_%s", nameDetector, sub_name);
    ...
    // ADC-2D LowGain Cut ------------------------------------------
    target_id = getUniqueID(kParaVC, 0, kLowGain, 100);
    sub_name = "CLG";
    // Add to the top directory
    title = Form("%s_%s", nameDetector, sub_name);
    sub_dir->Add(createTH2(++target_id, title, // 1 origin
  ...
  // MultiHitTdc-2D -------------------------------------------------------
  {
    ...
    Int_t target_id = getUniqueID(kParaVC, 0, kMultiHitTdc, 20);
    const char *sub_name = "MultiHitTdc";
    // Add to the top directory
    const char *title = NULL;
    title = Form("%s_%s", nameDetector, sub_name);
  ...
  // ADC HighGain vs TOT ----------------------------------------------
  {
    ...
    Int_t target_id = getUniqueID(kParaVC, 0, kHighGainvsTOT, 30);
    const char *sub_name = "HGvsTOT";
    // Add to the top directory
    TString strSubSubDir = Form("%s_%s", nameDetector, sub_name);
    const char *nameSubSubDir = strSubSubDir.Data();
    TList *sub_sub_dir = new TList;
    sub_sub_dir->SetName(nameSubSubDir);
    for (Int_t j = 0; j < NumOfSegVMEEASIROC; ++j)
  ...
  // TDC vs TOT ----------------------------------------------
  {
    ...
    Int_t target_id = getUniqueID(kParaVC, 0, kTOTXTDC, 30);
    const char *sub_name = "TDCvsTOT";
    // Add to the top directory
    for (Int_t j = 0; j < NumOfSegVMEEASIROC; ++j)
    {
  ...
  // Hit parttern -----------------------------------------------
  {
    ...
    Int_t target_id = getUniqueID(kParaVC, 0, kHitPat, 0);
    const char *sub_name = "HitPat";
    // Add to the top directory
    const char *title = NULL;
    title = Form("%s_%s", nameDetector, sub_name);
  ...
  // Multiplicity -------------------------------------------------
  {
    ...
    Int_t target_id = getUniqueID(kParaVC, 0, kMulti, 0);
    const char *sub_name = "Multiplicity";
    ...
    for (Int_t j = 0; j < 5; ++j)
    {
      const char *title = NULL;
      title = Form("%s_%s_%d", nameDetector, sub_name, j);
      sub_sub_dir->Add(createTH1(++target_id, title, // 1 origin
                                 5, 0, 5,
                                 "Multiplicity", ""));
    }
    sub_dir->Add(sub_sub_dir);
